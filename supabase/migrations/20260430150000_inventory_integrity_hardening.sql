...
begin
  ...
  if p_adjustment_type in ('reduce', 'set', 'manual_correction', 'out_of_stock')
    and normalized_notes is null then
    raise exception 'Reason/notes are required for this inventory adjustment.';
  end if;
  ...
  case p_adjustment_type
    ...
    when 'reduce' then
      ...
      if resolved_delta > resolved_quantity_before then
        raise exception 'Cannot reduce inventory below zero. Available quantity: %, requested reduction: %.', resolved_quantity_before, resolved_delta;
      end if;
      resolved_quantity_after := resolved_quantity_before - resolved_delta;
    ...
    when 'manual_correction' then
      if p_next_quantity is not null then
        ...
        if resolved_quantity_before + resolved_delta < 0 then
          raise exception 'Cannot reduce inventory below zero. Available quantity: %, requested reduction: %.', resolved_quantity_before, abs(resolved_delta);
        end if;
        resolved_quantity_after := resolved_quantity_before + resolved_delta;
        resolved_delta := resolved_quantity_after - resolved_quantity_before;
      end if;
