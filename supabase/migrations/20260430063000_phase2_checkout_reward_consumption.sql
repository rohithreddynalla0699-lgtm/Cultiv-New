...
declare
  ...
  v_result jsonb;
  v_selected_rewards jsonb;
  v_consumed_entitlement_id uuid;
begin
  ...
  v_selected_rewards := coalesce(v_payment.metadata->'selected_reward_entitlements', v_order->'selected_reward_entitlements', '[]'::jsonb);
  ...
  if jsonb_typeof(v_selected_rewards) <> 'array' then
    update public.customer_payments
      set status = 'orphaned',
          failure_message = 'Stored selected reward payload is invalid.',
          updated_at = now()
    where payment_id = p_payment_id;
    raise exception 'Stored selected reward payload is invalid.';
  end if;
  ...
  for v_selected_reward in select value from jsonb_array_elements(v_selected_rewards)
  loop
    v_consumed_entitlement_id := null;

    begin
      v_consumed_entitlement_id := nullif(trim(coalesce(v_selected_reward->>'entitlement_id', '')), '')::uuid;
    exception
      when invalid_text_representation then
        raise exception 'Stored selected reward payload is invalid.';
    end;

    if v_consumed_entitlement_id is null then
      raise exception 'Stored selected reward payload is invalid.';
    end if;
    ...
    update public.customer_reward_entitlements
      set status = 'used',
          ...
          order_id = v_order_id,
          updated_at = v_now
    where id = v_consumed_entitlement_id
      and customer_id = v_payment.customer_id
      and status = 'available'
      and (expires_at is null or expires_at > v_now)
      and (
        nullif(trim(coalesce(v_selected_reward->>'reward_id', '')), '') is null
        or reward_id = nullif(trim(coalesce(v_selected_reward->>'reward_id', '')), '')::uuid
      )
    returning id into v_consumed_entitlement_id;

    if v_consumed_entitlement_id is null then
      raise exception 'One or more selected rewards are no longer available.';
    end if;
  end loop;
