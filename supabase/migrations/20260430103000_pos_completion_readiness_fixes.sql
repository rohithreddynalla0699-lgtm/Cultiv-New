create or replace function public.award_loyalty_for_completed_order(
  p_order_id uuid,
  p_award_source text default 'order_completion'
)
returns jsonb
language plpgsql
security definer
...
declare
  ...
  v_earn_rate integer := 10;
  v_points_expiry_days integer := 90;
  v_points integer := 0;
  v_entry_id uuid;
...
begin
  ...
  select
    greatest(coalesce(earn_rate_rupees_per_point, 10), 1),
    greatest(coalesce(points_expiry_days, 90), 1)
    into v_earn_rate, v_points_expiry_days
  from public.reward_program_settings
  order by created_at asc
  limit 1;

  v_earn_rate := greatest(coalesce(v_earn_rate, 10), 1);
  v_points_expiry_days := greatest(coalesce(v_points_expiry_days, 90), 1);
  ...
  if exists (
    ...
    v_available_points := public.sync_customer_reward_points(v_order.customer_id);
    return jsonb_build_object(
      'success', true,
      'awarded', false,
      'alreadyAwarded', true,
  ...
  v_points := floor(coalesce(v_order.total_amount, 0) / v_earn_rate);
  ...
  insert into public.loyalty_points_ledger (
    ...
    v_points,
    v_now,
    v_now + make_interval(days => v_points_expiry_days),
    jsonb_build_object(
      'source', nullif(trim(coalesce(p_award_source, '')), ''),
      'total_amount', v_order.total_amount,
      'earn_rate_rupees_per_point', v_earn_rate,
      'points_expiry_days', v_points_expiry_days
    ),
    v_now,
    v_now
  )
...
revoke all on function public.award_loyalty_for_completed_order(uuid, text) from public, anon, authenticated;
grant execute on function public.award_loyalty_for_completed_order(uuid, text) to service_role;
...
begin
  ...
  for v_attempt in 1..3 loop
    ...
    begin
      insert into public.orders (
        ...
        'walk_in',
        'walk_in',
        -- POS counter orders are created as completed because payment and handoff happen at counter.
        -- Do not use this path for kitchen queue orders.
        'completed',
        p_store_id,
  ...
  if p_customer_id is not null then
    perform public.award_loyalty_for_completed_order(v_order_id, 'pos_counter_completion');
  end if;
...
revoke all on function public.create_internal_pos_order_with_payment(
  uuid, uuid, uuid, text, text, text, text, text, numeric, numeric, numeric, numeric, jsonb
