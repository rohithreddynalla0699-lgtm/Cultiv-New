...
export function AdminSummaryScreen() {
  ...
  const readyOrders = useMemo(
    () => scopedOrders.filter((order) => getAdminOrderBoardStatus(order) === 'ready'),
    [scopedOrders],
  );
  ...
  const lowStockItems = useMemo(
    () => scopedInventory.filter((item) => item.status === 'low_stock' || item.status === 'out_of_stock'),
    [scopedInventory],
  );
  const onShiftEmployees = useMemo(
    () => scopedEmployees.filter((employee) => employee.status === 'on_shift'),
    [scopedEmployees],
  );
  ...
  const hourlySeries = useMemo(() => {
    const buckets = Array.from({ length: 6 }, (_, index) => {
      ...
      const matching = todayOrders.filter((order) => {
        const createdHour = new Date(order.createdAt).getHours();
        return createdHour >= hour && createdHour < hour + 2;
      });
  ...
  return (
    ...
    >
      <SectionHeader
        ...
        description={`A calm overview of order pace, shift coverage, and stock pressure for ${scopeLabel}.`}
        action={
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        <StatCard
          ...
          hint="Orders that can be handed off now."
          icon={<ShoppingBag className="h-5 w-5" />}
        />
        <StatCard
          label="Active orders"
          value={`${activeOrders.length}`}
          hint="Orders still moving through assembly or pickup."
          icon={<ClipboardList className="h-5 w-5" />}
        />
        <StatCard
          label="Low stock items"
          value={`${lowStockItems.length}`}
          hint="Items needing attention this shift."
          icon={<Package2 className="h-5 w-5" />}
        />
        <StatCard
          label="On shift now"
          value={`${onShiftEmployees.length}`}
          hint="Team members currently clocked in."
          icon={<Users className="h-5 w-5" />}
        />
        <StatCard
          label="Today’s orders"
          value={`${todayOrders.length}`}
          hint="Pickup orders placed today."
          icon={<ShoppingBag className="h-5 w-5" />}
        />
        <StatCard
          label="Today’s revenue"
          value={formatCurrency(todayRevenue)}
          hint="Collected from completed and active pickup orders."
            ...
            <div className="mt-4 space-y-3">
              {onShiftEmployees.slice(0, 5).map((employee) => (
                <div key={employee.id} className="rounded-2xl bg-[#F7FAF3] px-4 py-3">
                  ...
                  <p className="mt-1 text-sm text-foreground/58">
                    {employee.role}
                    {activeStoreScope === 'all' ? ` · ${getStoreName(employee.storeId)}` : ''}
                  </p>
                </div>
              ))}
            </div>
