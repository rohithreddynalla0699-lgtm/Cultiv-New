...
export function Header({ onOrderClick: _onOrderClick }: HeaderProps) {
  ...
  const rightUtilityClassName = "hidden xl:inline-flex items-center gap-2 rounded-2xl border border-primary/12 bg-white/88 px-3.5 py-2.5 text-sm font-medium text-foreground/72 transition-colors hover:border-primary/22 hover:text-foreground";
        ...
        <nav className="hidden lg:flex items-center gap-7 justify-self-center">
          <NavLink to="/" className={navClassName} onClick={(event) => handleNavTopScroll(event, '/')}>
            {({ isActive }) => (
              <span className="relative inline-flex items-center gap-2 pb-1">
                Home
                {isActive && activeHomeTab === 'home' ? <span className="absolute inset-x-0 -bottom-1 h-px bg-primary" /> : null}
              </span>
            )}
          </NavLink>
          <NavLink to="/menu" className={navClassName} onClick={(event) => handleNavTopScroll(event, '/menu')}>
            {({ isActive }) => (
              <span className="relative inline-flex items-center gap-2 pb-1">
                Menu
                {isActive ? <span className="absolute inset-x-0 -bottom-1 h-px bg-primary" /> : null}
              </span>
            )}
          </NavLink>
          ...
          <button type="button" onClick={() => jumpToHomeSection('journal')} className="text-sm text-foreground/72 transition-colors hover:text-foreground">
            <span className="relative inline-flex items-center gap-2 pb-1">
              Journal
              {location.pathname === '/' && activeHomeTab === 'journal' ? <span className="absolute inset-x-0 -bottom-1 h-px bg-primary" /> : null}
            </span>
          </button>
          <button type="button" onClick={() => jumpToHomeSection('about')} className="text-sm text-foreground/72 transition-colors hover:text-foreground">
            <span className="relative inline-flex items-center gap-2 pb-1">
              About
              {location.pathname === '/' && activeHomeTab === 'about' ? <span className="absolute inset-x-0 -bottom-1 h-px bg-primary" /> : null}
            </span>
          </button>
        </nav>

        <div className="flex items-center gap-2 justify-self-end">
          <button type="button" onClick={() => jumpToHomeSection('locations')} className={rightUtilityClassName}>
            <MapPin className="h-4 w-4 text-primary" />
            <span className="hidden 2xl:inline">Siddipet Central</span>
          ...
          {user ? (
            <div className="relative" ref={menuRef}>
              ...
              >
                ...
                <ChevronDown className="hidden sm:block h-4 w-4 text-foreground/40" />
              </button>

              {showUserMenu && (
                <div className="absolute right-0 top-full mt-3 w-56 rounded-2xl bg-card shadow-lg border border-border py-2 z-50">
                  <Link
                    to="/profile"
